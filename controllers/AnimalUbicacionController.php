...
class AnimalUbicacionController
{
    ...
    public function listar(): void
    {
        ...
        try {
            $rows = $this->model->listar($limit, $offset, $incluir, $animalId, $fincaId, $apriscoId, $areaId, $desde, $hasta, $soloActivas);
    ...
    public function mostrar(array $params): void
    {
        ...
        try {
            ...
            if (!$row) $this->jsonResponse(false, 'Ubicación no encontrada.', null, 404);
    ...
    public function actual(array $params): void
    {
        ...
        try {
            ...
            if (!$row) $this->jsonResponse(false, 'No hay ubicación activa.', null, 404);
    ...
    public function actualizar(array $params): void
    {
        ...
        try {
            ...
            $this->jsonResponse(true, 'Ubicación actualizada correctamente.', ['updated' => $ok]);
        } catch (InvalidArgumentException $e) {
            $this->jsonResponse(false, $e->getMessage(), null, 400);
        } catch (RuntimeException $e) {
            $this->jsonResponse(false, $e->getMessage(), null, 409);
    ...
    public function cerrar(array $params): void
    {
        ...
        $in = $this->getJsonInput();
        $fechaHasta = (string)($in['fecha_hasta'] ?? '');
        if ($fechaHasta === '') {
            $this->jsonResponse(false, 'El campo fecha_hasta es obligatorio.', null, 400);
        }

        try {
            $ok = $this->model->cerrar($id, $fechaHasta);
            $this->jsonResponse(true, 'Ubicación cerrada correctamente.', ['closed' => $ok]);
        } catch (InvalidArgumentException $e) {
            $this->jsonResponse(false, $e->getMessage(), null, 400);
        } catch (Throwable $e) {
